...
export function getOrAssignVariant(): AbVariant {
  // Guard against SSR — sessionStorage is only available in the browser.
  if (typeof window === 'undefined') return 'A';
  const stored = sessionStorage.getItem(AB_KEY) as AbVariant | null;
  if (stored && VARIANTS.includes(stored)) return stored;
  const picked = VARIANTS[Math.floor(Math.random() * 3)];
  sessionStorage.setItem(AB_KEY, picked);
  return picked;
}
